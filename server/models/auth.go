...
func (m AuthModel) CreateAuth(userid int64, td *TokenDetails) error {
	...
	errAccess := store.GetRedis().Set(ctx, td.AccessUUID, strconv.Itoa(int(userid)), at.Sub(now)).Err()
	if errAccess != nil {
		return errAccess
	}
	errRefresh := store.GetRedis().Set(ctx, td.RefreshUUID, strconv.Itoa(int(userid)), rt.Sub(now)).Err()
	if errRefresh != nil {
		return errRefresh
	}
	return nil
...
func (m AuthModel) FetchAuth(authD *AccessDetails) (int64, error) {
	userid, err := store.GetRedis().Get(ctx, authD.AccessUUID).Result()
	if err != nil {
		return 0, err
	}
...
func (m AuthModel) DeleteAuth(givenUUID string) (int64, error) {
	deleted, err := store.GetRedis().Del(ctx, givenUUID).Result()
	if err != nil {
		return 0, err
	}
	return deleted, nil
